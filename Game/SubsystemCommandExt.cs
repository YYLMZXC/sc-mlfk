				...
				ComponentCreature componentCreature = entity.FindComponent<ComponentCreature>();
				{
					foreach (string key2 in m_creatureDataChange[key].Keys)
					{
						...
						case "health":
							...
							break;
						case "attack-power":
							componentCreature.Entity.FindComponent<ComponentMiner>(true).AttackPower = (int)obj;
							break;
		...
		public SubmitResult ChangeBlockData(CommandData commandData)
		{
			...
			if (commandData.Type == "behaviors")
			{
				...
				try
				{
					...
					{
						SubsystemBlockBehavior[] value;
						if (m_originBehaviorsById.TryGetValue(num, out value))
						{
					...
					for (int i = 0; i < array2.Length; i++)
					{
						...
						if (!flag)
						{
							subsystemCommand.ShowSubmitTips(string.Format("{0}行为不存在或不可用", array2[i]));
							return SubmitResult.Fail;
						}
						flag = false;
					}
					...
					{
						SubsystemBlockBehavior item = base.Project.FindSubsystem<SubsystemBlockBehavior>(text3.Trim(), true);
						list.Add(item);
					}
					SubsystemBlockBehavior[] value2;
					if (!m_originBehaviorsById.TryGetValue(num, out value2))
					{
			...
			foreach (ComponentBody item in dynamicArray)
			{
				...
				if (value3 != null)
				{
					...
					case "health":
						...
						break;
					case "attack-power":
						componentCreature.Entity.FindComponent<ComponentMiner>(true).AttackPower = num3;
						break;
					case "attack-resilience":
