...
namespace Mlfk
{
    public class DataHandle
    {
        ...
        public static string ExpressionHandle(string str)
        {
            ...
            do
            {
                ...
                else
                {
                    ...
                    {
                        while (!tStack2.IsEmpty() && GetLevel(text2) <= GetLevel(tStack2.GetTop()?.ToString() ?? ""))
                        {
            ...
            double x = double.Parse(tStack.Pop()?.ToString() ?? "");
        ...
        public static int Calculate(TStack numStatck, TStack operStatck)
        {
            ...
            {
                num = int.Parse(numStatck.Pop()?.ToString() ?? "");
            }
        ...
        public static object Raycast(Ray3 ray, ComponentMiner componentMiner)
        {
            ...
            TerrainRaycastResult? terrainRaycastResult = componentMiner.m_subsystemTerrain.Raycast(start, end, useInteractionBoxes: true, skipAirBlocks: true, (int value, float distance) => true);
