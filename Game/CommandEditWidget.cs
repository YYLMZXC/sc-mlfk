	...
	public class CommandEditWidget : CanvasWidget
	{
		...
		public override void Update()
		{
			if (m_commandNameButton.IsClicked)
			{
				...
				Vector2 value;
				if (!ScrollPosition.TryGetValue(m_position, out value))
				{
					value = Vector2.Zero;
					ScrollPosition[m_position] = value;
				}
				DialogsManager.ShowDialog(m_componentPlayer.GuiWidget, new CommandListSelectionDialog(true, instructions.Values, 50f, value.X, (object i) => new LabelWidget
				{
				...
				}, delegate(TextBoxWidget textBox, ListPanelWidget listWidget)
				{
					...
					{
						string text = InstructionManager.GetDisplayName(value4.Name) + value4.About;
						if (text.Contains(textBox.Text))
						{
			...
			if (m_commandTypeButton.IsClicked)
			{
				...
				Vector2 value2;
				if (!ScrollPosition.TryGetValue(m_position, out value2))
				{
					value2 = Vector2.Zero;
					ScrollPosition[m_position] = value2;
				}
				DialogsManager.ShowDialog(m_componentPlayer.GuiWidget, new CommandListSelectionDialog(true, instruction.Types, 50f, value2.Y, (object type) => new LabelWidget
				{
					Text = (string)type,
				...
				}, delegate(object type, float scroll)
				{
					...
					if (type != null && m_type != (string)type)
					{
						m_type = (string)type;
						m_commandTypeLabel.Text = GetAbridgeText(m_type);
						CommandData commandData2 = new CommandData(Point3.Zero, string.Empty);
						SetCommandDataValue(commandData2);
						ChangeExplains();
						GetDynamicCommanndWidget();
						SetCommandWidgetValue(commandData2);
					}
				}, delegate(TextBoxWidget textBox, ListPanelWidget listWidget)
				{
					listWidget.ClearItems();
			...
			if (m_coordinateButton.IsClicked)
			{
				...
				DialogsManager.ShowDialog(m_componentPlayer.GuiWidget, new CommandListSelectionDialog(false, items, 50f, 0f, (object coord) => new LabelWidget
				{
					Text = (string)coord,
				...
				}, delegate(object coord, float scroll)
				{
					if (coord != null)
					{
						string text3 = ((string)coord).Replace(" ", "").Split(':')[0];
						if (m_coordinate != text3)
						{
							m_coordinate = text3;
			...
			{
				CommandData commandData4 = SaveCommandData(false);
				if (commandData4.Mode == WorkingMode.Default)
				{
					SubmitResult submitResult = m_subsystemCmdRodBehavior.m_subsystemCommand.Submit(commandData4.Name, commandData4, false);
					switch (submitResult)
					{
				...
				else if (commandData4.Mode == WorkingMode.Condition)
				{
					ShowEditTips("快速指令功能不支持条件指令");
				}
				else if (commandData4.Mode == WorkingMode.Variable)
				{
			...
			foreach (string key in m_commandWidgetDatas.Keys)
			{
				...
				else
				{
					...
					else
					{
						...
						{
							DialogsManager.ShowDialog(null, new CommandListSelectionDialog(false, instructionOption, 50f, 0f, (object option) => new LabelWidget
							{
		...
		public void SetCommandWidgetValue(CommandData commandData)
		{
			foreach (string key in m_commandWidgetDatas.Keys)
			{
				...
				string value;
				if (commandData.DataText.TryGetValue(array[1], out value))
				{
					if (array[0] == "TextBox")
					{
		...
		{
			int result;
			if (int.TryParse(dataText, out result))
			{
		...
		{
			int result;
			if (int.TryParse(dataText, out result) && (float)result >= sliderWidget.MinValue && (float)result <= sliderWidget.MaxValue)
			{
		...
		public CommandData SaveCommandData(bool collection)
		{
			...
			else
			{
				...
				m_subsystemTerrain.ChangeCell(m_position.X, m_position.Y, m_position.Z, CommandBlock.SetWorkingMode(cellValue, commandData.Mode), true, (ComponentMiner)null);
			}
