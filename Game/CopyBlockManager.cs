    ...
    public class CopyBlockManager
    {
        ...
        public void ClearBlockArea(bool applyChangeCell = false)
        {
            foreach (Point3 key in CopyBlockDatas.Keys)
            {
                ...
                {
                    SubsystemCommandDef.m_subsystemBlockEntities.GetBlockEntity(key.X, key.Y, key.Z)?.Entity.FindComponent<ComponentInventoryBase>().m_slots.Clear();
                    SubsystemCommandDef.m_subsystemTerrain.ChangeCell(key.X, key.Y, key.Z, 0);
                }
