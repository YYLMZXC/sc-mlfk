...
namespace Game
{
    public class InstructionManager
    {
        ...
        public static void Initialize()
        {
            foreach (ContentInfo item in ContentManager.List())
            {
                ...
                if (!string.IsNullOrEmpty(text))
                {
                    ...
                    {
                        SetInstructions(array[2], array[4], iscon: false);
                        SetInstructions(array[6], array[8], iscon: true);
                    }
                }
            }
        ...
        public static Instruction GetInstruction(string name, bool iscon)
        {
            ...
            if (!iscon)
            {
                if (FunInstructions.TryGetValue(name, out var value))
                {
                    return value;
                }
            }
        ...
        {
            Instruction instruction = GetInstruction(name, iscon);
            if (instruction != null && instruction.Demos.TryGetValue(type, out var value))
            {
                return value;
            }
        ...
        {
            Instruction instruction = GetInstruction(name, iscon);
            if (instruction != null && instruction.Details.TryGetValue(type, out var value))
            {
        ...
        {
            Instruction instruction = GetInstruction(name, iscon);
            if (instruction != null && instruction.Options.TryGetValue(type + "$" + para, out var value))
            {
        ...
        public static void SetInstructions(string classify1, string classify2, bool iscon)
        {
            ...
            foreach (string text4 in array5)
            {
                ...
                if (!dictionary.TryGetValue(array7[0], out var value2))
                {
        ...
        {
            Instruction instruction = GetInstruction("texture", iscon: false);
            if (instruction != null)
            {
                string text = string.Empty;
            ...
            Instruction instruction2 = GetInstruction("audio", iscon: false);
            ...
            Instruction instruction3 = GetInstruction("model", iscon: false);
