	...
	public class NotesWidget : CanvasWidget
	{
		...
		public override void Update()
		{
			...
			if (m_editButton.IsClicked)
			{
				...
				KeyValuePair<string, string> n = m_notes.ElementAt(m_index);
			...
			if (m_removeButton.IsClicked)
			{
				...
				KeyValuePair<string, string> n2 = m_notes.ElementAt(m_index);
				DialogsManager.ShowDialog(m_componentPlayer.GuiWidget, new MessageDialog(string.Format("是否删除标题为{0}的笔记", n2.Key), string.Empty, "确定", "取消", delegate(MessageDialogButton button)
				{
					if (button == MessageDialogButton.Button1)
					{
						m_notes.Remove(n2.Key);
						m_index = 0;
						m_contentLabel.Text = ShowText(m_index);
					}
				}));
			...
			if (m_addButton.IsClicked)
			{
				DialogsManager.ShowDialog(m_componentPlayer.GuiWidget, new TextBoxDialog("请输入笔记标题", string.Empty, 256, delegate(string title)
				{
					try
					{
						...
						{
							string value;
							if (m_notes.TryGetValue(title, out value))
							{
								m_notes[title] = value;
							}
							else
