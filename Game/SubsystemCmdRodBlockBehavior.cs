...
namespace Game
{
    public class SubsystemCmdRodBlockBehavior : SubsystemBlockBehavior, IDrawable, IUpdateable
    {
        ...
        public override int[] HandledBlocks => new int[1] { 334 };
        ...
        public override bool OnUse(Ray3 ray, ComponentMiner componentMiner)
        {
            ...
            string text = $"\n视角: 水平方向:{playerEyesAngle.X},垂直方向:{playerEyesAngle.Y}";
            if (obj is TerrainRaycastResult)
            {
                CellFace cellFace = ((TerrainRaycastResult)obj).CellFace;
                ...
                empty = $"方块ID:{num2}；特殊值:{num3}；方块值:{num}\n标准坐标:({cellFace.X},{cellFace.Y},{cellFace.Z})；方块面:{cellFace.Face}";
                switch (num2)
                {
                    case 333:
                        componentMiner.ComponentPlayer.ComponentGui.DisplaySmallMessage(empty, Color.LightBlue, blinking: false, playNotificationSound: false);
                        return false;
                    case 227:
                        {
                ...
                if (num4 != -1)
                {
                    ...
                    text3 = $"\n检测到掉落物的ID:{num7}；特殊值:{num8}；方块值:{num4}";
                    if (num7 == 203)
                    {
                ...
                componentMiner.ComponentPlayer.ComponentGui.DisplaySmallMessage(empty + text + text2 + text3, m_color, blinking: false, playNotificationSound: false);
                return true;
            }
            ...
            if (obj is BodyRaycastResult)
            {
                ...
                if (componentBody != null)
                {
                    ...
                    if (componentCreature != null)
                    {
                        ...
                        text5 += $"；名称:{displayName}；\n血量:{num9}%；质量:{text6}；\n位置:({bodyPoint.ToString()})；碰撞箱:{text7}；";
                        text5 += $"\n飞行速度:{MathUtils.Round(flySpeed * 10f) / 10f}；行走速度:{MathUtils.Round(walkSpeed * 10f) / 10f}；跳跃速度:{MathUtils.Round(jumpSpeed * 10f) / 10f}";
                    }
                    else if (componentDamage != null)
                    {
                        ...
                        text5 += $"；血量:{num10}%；\n攻击抗性:{attackResilience}；质量:{text8}；\n位置:({bodyPoint.ToString()})；碰撞箱:{text9}；";
                    }

                    SetPointData(bodyPoint);
                    m_recordPosition = bodyPoint;
                    m_recordEntityName = text4;
                    componentMiner.ComponentPlayer.ComponentGui.DisplaySmallMessage(text5, m_color, blinking: false, playNotificationSound: false);
                    return true;
                }
            }
            else
            {
                if (obj is MovingBlocksRaycastResult)
                {
                    ...
                    text10 = text10 + "；所在位置:" + $"({movingBlockSet.Position.ToString()})";
                    SetPointData(new Point3(movingBlockSet.Position));
                    m_recordPosition = new Point3(movingBlockSet.Position);
                    componentMiner.ComponentPlayer.ComponentGui.DisplaySmallMessage(text10, m_color, blinking: false, playNotificationSound: false);
                    return true;
                }
                ...
                m_recordEyes = playerEyesAngle;
                componentMiner.ComponentPlayer.ComponentGui.DisplaySmallMessage($"玩家名:player; 玩家坐标:({bodyPoint2.ToString()})" + text, m_color, blinking: false, playNotificationSound: false);
            }
        ...
        public override bool OnEditInventoryItem(IInventory inventory, int slotIndex, ComponentPlayer componentPlayer)
        {
            if (componentPlayer.ComponentGui.ModalPanelWidget is CommandEditWidget)
            {
                ...
                CommandEditWidget.GuiWidgetControl(componentPlayer, button: true);
                componentPlayer.ComponentGui.ModalPanelWidget = null;
            }
            else
            {
                componentPlayer.ComponentGui.ModalPanelWidget = new CommandEditWidget(base.Project, componentPlayer, Point3.Zero, fastMode: true);
            }
        ...
        public override void Load(ValuesDictionary valuesDictionary)
        {
            ...
            m_subsystemCommand = base.Project.FindSubsystem<SubsystemCommand>(throwOnError: true);
        ...
        public void Update(float dt)
        {
            ...
            if (m_withdrawButton.IsClicked)
            {
                ...
                {
                    m_subsystemCommand.m_componentPlayer.ComponentGui.DisplaySmallMessage("请开启撤回模式", m_color, blinking: false, playNotificationSound: false);
                }
            }
            ...
            foreach (string key in m_commandPoints.Keys)
            {
                if (m_commandPoints[key].Button.IsClicked)
                {
                    ...
                    m_subsystemCommand.m_componentPlayer.ComponentGui.DisplaySmallMessage(text, Color.Yellow, blinking: false, playNotificationSound: false);
                }
            }
        }
                ...
                if (m_recordPosition.HasValue)
                {
                    ...
                    if (value == recordPosition)
                    {
                        Time.QueueTimeDelayedExecution(Time.RealTime + 0.10000000149011612, delegate
                        {
                            ...
                            m_subsystemCommand.Submit(commandData.Name, commandData, Judge: false);
                            m_subsystemCommand.m_componentPlayer.ComponentGui.DisplaySmallMessage($"已提交指令:{commandData.Name}${commandData.Type}", Color.Yellow, blinking: false, playNotificationSound: false);
                        });
                        return;
                    }
                }
