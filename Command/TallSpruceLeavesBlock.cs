...
namespace Command
{
	public class TallSpruceLeavesBlock : Game.TallSpruceLeavesBlock
	{
		...
		{
			if (SubsystemCommandDef.DisplayColorBlock && value != 225)
			{
		...
		public override void GenerateTerrainVertices(BlockGeometryGenerator generator, TerrainGeometry geometry, int value, int x, int y, int z)
		{
			...
			{
				color = m_blockColorsMap.Lookup(generator.Terrain, x, y, z);
			}
			Color color2 = color;
		...
		public override BlockDebrisParticleSystem CreateDebrisParticleSystem(SubsystemTerrain subsystemTerrain, Vector3 position, int value, float strength)
		{
			...
			if (IsDefaultColor(color))
			{
				color = m_blockColorsMap.Lookup(subsystemTerrain.Terrain, Terrain.ToCell(position.X), Terrain.ToCell(position.Y), Terrain.ToCell(position.Z));
			}
			Color color2 = color;
